...
# 的 specialist（test-gap-analysis；speak-human-tw 與 ai-level-check 同類，但兩者的
# description 精準度未經 skills/auditing-skill-folder/evals/cases.jsonl 量測，分類是判斷
# 不是實證。ai-level-check 評的是「使用者用 AI 的成熟度」，不是任何開發階段的方法，
# 放進 S0 會讓 kernel 多背一條與交付無關的路由）。
