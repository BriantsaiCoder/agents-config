...
section_has "Claude announces the implementation split before starting" Claude '實作分工通報.*MUST 在開始.*已授權實作前.*告知實作者（Fable 直接實作或 configured `implementer`）.*負責範圍與選擇原因' "$host_adapters_ref"
