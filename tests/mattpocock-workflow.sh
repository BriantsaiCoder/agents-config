...
section_has "Claude derives the coordinator role from configured settings" Claude 'Claude main role = .*configured `model` with effort `high`.*plan mode' "$host_adapters_ref"
