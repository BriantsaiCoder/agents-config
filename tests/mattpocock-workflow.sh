...
has "delegation keeps independently parallel scope as the default" 'Serial routing 例外以外，任何 delegation 機制只在工作可獨立平行.*使用' "$delegation_ref"
has "delegation scopes the serial exception to Codex and Claude" '唯一 eligibility 例外.*Astra→Sol serial implementation routing.*Fable→Opus serial implementation routing' "$delegation_ref"
has "delegation keeps the serial exception out of Copilot" '此例外不適用 Copilot' "$delegation_ref"
...
section_has "Codex keeps Astra as coordinator" Codex 'gpt-6-astra.*reasoning `high`.*Plan Mode' "$host_adapters_ref"
...
section_has "Codex automatically routes authorized operations to Sol" Codex 'MUST automatically delegate implementation.*integration.*testing.*fixes.*closeout.*implementer.*gpt-5\.6-sol.*xhigh' "$host_adapters_ref"
section_has "Codex forbids implementer recursion" Codex 'same-work recursion is forbidden' "$host_adapters_ref"
section_has "Codex blocks instead of substituting an unavailable implementer" Codex 'implementer.*UNAVAILABLE blocker.*does not authorize Astra implementation or model substitution' "$host_adapters_ref"
section_has "Codex requires Astra to reverify implementer evidence" Codex 'Astra may run checks.*Sol report is not completion evidence.*reverify' "$host_adapters_ref"
...
section_has "Claude blocks instead of substituting an unavailable implementer" Claude 'implementer.*UNAVAILABLE blocker.*does not authorize Fable implementation or model substitution' "$host_adapters_ref"
