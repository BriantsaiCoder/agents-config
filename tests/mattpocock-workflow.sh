...
section_has "Claude reuses the original implementer for same-scope follow-up" Claude 'host 有 `SendMessage` 時 MUST 優先.*續用原 implementer.*只有原代理不可用.*新的獨立任務.*建立新代理' "$host_adapters_ref"
section_has "Claude falls back to a re-sent packet without SendMessage" Claude 'host 無 `SendMessage`.*重發 packet.*delegation\.md.*changed files.*失敗.*MUST NOT 逐字重貼' "$host_adapters_ref"
section_has "Claude defaults new subagents to no inherited parent context" Claude 'MUST 預設不繼承父對話.*`subagent_type: "fork"`' "$host_adapters_ref"
section_has "Claude forbids fork when delegating to the configured implementer" Claude 'configured `implementer`.*MUST NOT 用 fork' "$host_adapters_ref"
