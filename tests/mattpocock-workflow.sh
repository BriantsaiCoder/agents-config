...
has "delegation keeps independently parallel scope as the default" 'Codex 例外以外，任何 delegation 機制只在工作可獨立平行.*使用' "$delegation_ref"
has "delegation scopes the serial exception to Codex" 'Codex 的唯一 eligibility 例外.*Astra→Sol serial implementation routing' "$delegation_ref"
has "delegation keeps the serial exception out of Claude and Copilot" '此例外不適用 Claude／Copilot' "$delegation_ref"
