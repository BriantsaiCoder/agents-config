...
if [ -d "$AGENTS/skills" ]; then
  ...
  expect_vnd="agent-browser playwright-best-practices security-audit tailwind-v4-shadcn ui-ux-pro-max"
  ...
  if [ -r "$AGENTS/vendored-skills.lock" ]; then
    ...
    check "generic provenance lock 的 skill 數量" "6" "$generic_count"
