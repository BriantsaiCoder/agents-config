...
# Keep direct CLI execution from mutating the pinned vendored tree.
...
def _search_results(search_result: dict) -> list:
    """Return search rows or fail instead of generating plausible defaults."""
    if search_result.get("error"):
        raise RuntimeError(search_result["error"])
    results = search_result.get("results", [])
    if results and results[0].get("_error"):
        raise RuntimeError(results[0]["_error"])
    return results
...
class DesignSystemGenerator:
    ...
    def _load_reasoning(self) -> list:
        ...
        if not filepath.exists():
            raise FileNotFoundError(f"Reasoning data not found: {filepath}")
    ...
    def generate(self, query: str, project_name: str = None,
        ...
        product_results = _search_results(product_result)
        category = "General"
        if product_results:
        ...
        style_results = _search_results(search_results.get("style", {}))
        color_results = _search_results(search_results.get("color", {}))
        typography_results = _search_results(search_results.get("typography", {}))
        landing_results = _search_results(search_results.get("landing", {}))
        ...
        if motion_info:
            ...
            motion_matches = _search_results(motion_result)
...
def generate_design_system(query: str, project_name: str = None, output_format: str = "ascii",
    ...
    Args:
        ...
        force: If True, overwrite existing requested files; otherwise each
               existing Master/page target is preserved independently
...
def reject_managed_symlinks(path: Path, base_dir: Path) -> None:
    """Reject symlinks in the managed path below caller-selected base_dir."""
    current = path
    while current != base_dir:
        ...
        if parent == current:
            raise ValueError(f"Managed path is outside output directory: {path}")
        current = parent
...
def persist_design_system(design_system: dict, page: str = None, output_dir: str = None,
    ...
    Args:
        ...
        force: If True, overwrite existing requested files. If False (default),
               each existing Master/page target is preserved independently.
               Symlinks are always rejected.

    Returns:
        dict with created file paths and status. status is "skipped_exists" when
        every requested target already exists and force was not set.
    """
    ...
    page_file = pages_dir / f"{safe_slug(page, 'page')}.md" if page else None

    reject_managed_symlinks(master_file, base_dir)
    if page_file:
        reject_managed_symlinks(page_file, base_dir)

    master_exists = master_file.exists()
    page_exists = bool(page_file and page_file.exists())
    if not force and master_exists and (page_file is None or page_exists):
        existing = page_file if page_exists else master_file
        return {
            ...
            "created_files": [],
            "message": (
                f"{existing} already exists and was not modified. "
                "Read it first, then re-run with force=True / --force to overwrite."
            ),
        }
    ...
    if page_file and (force or not page_exists):
...
def _generate_intelligent_overrides(page_name: str, page_query: str, design_system: dict) -> dict:
    ...
    style_results = _search_results(style_search)
    ux_results = _search_results(ux_search)
    landing_results = _search_results(landing_search)
