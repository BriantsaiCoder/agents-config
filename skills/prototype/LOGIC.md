...
Put the actual logic — the bit that's answering the question — in a single `<script>` block written as a small, pure module. In a JavaScript codebase it may lift directly; in another language, carry the validated states, transitions, and invariants into a native implementation. The page around it is throwaway; the design it proves isn't.
...
One file, plain HTML/CSS/JS — no framework, no bundler, no server, everything inline so it opens by double-click and survives being emailed around. Keep it offline: no CDN, external network request, remote asset, secret, or production build-path dependency. Fixture state uses synthetic, non-sensitive data. Anyone should be able to run it by opening it.
...
Send them the file, or open it for them; delivery outside the local session follows the shared `dev-workflow` authorization gate. They'll click through the walkthroughs and free-play whenever they get to it; the interesting moments are when they say "wait, that shouldn't be possible" or "huh, I assumed X would be different" — those are the bugs in the _idea_, which is the whole point. If they want new actions or a new scenario, add them. Prototypes evolve.

### 5. Capture the answer and the prototype

Once the prototype has answered its question, capture the answer, then capture the prototype the way the [SKILL](SKILL.md) describes. The logic-specific mapping: lift JavaScript directly where compatible; otherwise reimplement the validated states, transitions, and invariants in the host language. The HTML shell rides along to the throwaway branch that keeps the prototype as a primary source — and being one self-contained file, it stays trivially re-runnable there.
