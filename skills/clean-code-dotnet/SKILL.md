...
- [ ] **Comments**: No regions, no dead code, explains WHY
 - [ ] **Maintainability**: Each method's Maintainability Index >= 50
 - [ ] **Cyclomatic Complexity**: Each method <= 20

---

## Maintainability & Complexity Metrics

- **Maintainability Index (MI)**: Aim for MI >= 50 for individual methods/functions. MI is a heuristic combining lines of code, cyclomatic complexity, and Halstead volume; use it as a guardrail for refactoring decisions.
- **Cyclomatic Complexity**: Keep per-method cyclomatic complexity <= 20. If a method exceeds this, refactor by extracting smaller functions, using guard clauses, or simplifying logic.

Recommended tools: Visual Studio Code extensions (CodeMetrics, Roslyn analyzers), SonarQube, or Visual Studio's Code Metrics to measure MI and complexity during CI or local reviews.
