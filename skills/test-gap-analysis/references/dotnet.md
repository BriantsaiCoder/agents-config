...
Reference data for analyzing .NET test code. Read by `test-gap-analysis` Step 1 to map the language-neutral mutation catalog onto the framework actually in use.
...
| Tag recognition | Strong — recognizes `[TestCategory]`, `[Trait]`, `[Category]`, `[Property]`; changing tags requires separate test-edit authorization and the stack's testing skill |
...
| Fail | `Assert.Fail()` | `Assert.Fail()` (availability follows the xUnit assertion package version, not the target framework) | `Assert.Fail()` | `Assert.Fail()` |
...
**xUnit exception semantics:** `Assert.Throws<T>` requires the exact exception type. Use `Assert.ThrowsAny<T>` only when derived exception types are intentionally accepted.
