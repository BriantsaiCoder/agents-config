...
- [INT-2] BUGFIX／既有 behavior 變更若有 stable、valuable 的 behavior seam，MUST 先有 failing regression test（RED→GREEN）；否則 MUST 以同一 minimal repro 留下 before／after evidence，並記錄不採 RED 的理由，MUST NOT 為流程新增低價值 seam。既有 public behavior seam 視為已確認，只有新增 seam 才需另向使用者確認。MUST NOT 為使測試或 gate 轉綠而弱化測試（放寬 assertion、加 skip／ignore、調高容差、刪除失敗測試）；測試看似錯誤時 surface 成 spec 問題，不得就地埋掉。MUST NOT 在同一步同時修改 test 與 implementation 以達成 GREEN——改一邊、跑、再改另一邊，同步修改會把 correctness 重新定義成剛好符合當下的 bug。觸發：BUGFIX 或既有 behavior 變更；後兩句另及任何為使測試或 gate 轉綠而修改 test 檔的動作。例外：無。驗證：RED evidence 早於 fix，或同一 repro 的 before／after evidence；assertion 變動對得到 spec／behavior 變更；改 test 與改 implementation 之間存在一次 run 的輸出（成對的 RED／GREEN 執行紀錄，而非事後自述）。
