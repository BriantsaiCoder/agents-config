...
Delegation 的**約束**不變，改變的只有**誰決定**。Host adapter 明定的 eligibility 例外以外，任何 delegation 機制只在工作可獨立平行且預期能實質改善品質或 wall-clock time 時使用；是否委派、何時委派、subagent 數量與是否平行 MUST 由 AI 自主判定並直接執行，MUST NOT 為此停下發問。除使用者明示的本次 scope、數量或成本限制外，不預設固定數量、併發、累計或 S 階段 quota；由 AI 按獨立性與預期效益決定，host/runtime 可用容量仍是技術上限。
