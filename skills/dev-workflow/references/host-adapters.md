...
- 兩軸 findings 處理完後 MUST 跑 `simplify`（Claude-only）當 apply pass：兩軸只報不修，這一步會動手改 [S5-3] baseline 落到實體 code 的部分。它在 review gate 之後動手，產出的 diff 卻一樣進 PR，所以 MUST 回 S4 重驗**並**依 `ledgers.md` 的「Closeout 後的新 commit」把該 diff 觸及的檔案重新納入 S5——只回 S4 等於讓一批 code 繞過 [S5-1]。該 pass 未產生改動時無此義務。
