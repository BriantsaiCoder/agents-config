...
- 用專屬 review agent 不豁免 `references/reviewer-template.md`：豁免的只有 prompt 區塊本身，其餘各節對 Claude 一樣有約束力，MUST 在維護 `code-review` 的 baseline 或判 S5 EXIT 時讀。不在此列舉是哪幾節——列舉會漏，新增的節就掉在外面。
