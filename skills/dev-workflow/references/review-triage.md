...
3. **Bot unavailable 的 independent fallback**
   ...
   - **允許側窮舉為一個 reason**：`review_actions_billing_or_quota`——Copilot review body 逐字是 billing 訊息，即 bot 這一側交不出 review，且須經合理等待與 retry 後仍為 `UNAVAILABLE`。窮舉本身是這條規則的形狀：這一行多出第二個 reason 名，`tests/mattpocock-workflow.sh` 先紅。
   - **除 `review_actions_billing_or_quota` 外，helper 回的任何 `UNAVAILABLE` 一律不得 fallback**，含日後新增的 reason：`review_request_failed`（`requested_reviewers` POST 失敗；422 參數錯、403／404 權限、429 rate limit、5xx、網路中斷共用同一個失敗，單看它分不出是哪一種，MUST 修 helper 或 retry）、probe 失敗（`repo_probe_failed`、`pr_probe_failed`、`review_probe_failed`、`requested_reviewer_probe_failed`）、欄位或值的形狀檢查失敗（`*_fields_unparsable`、`*_not_numeric`）、head 無法確認、thread probe／pagination 不完整，都不得 fallback。（2026-08-24, PR #85；本次把 request 失敗移出允許側見 #87）
