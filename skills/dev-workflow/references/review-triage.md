...
3. **Bot unavailable 的 independent fallback**
   ...
   - fallback 只適用於 **bot 這一側交不出 review**、經合理等待與 retry 後仍為 `UNAVAILABLE` 的情形。helper 端目前有兩個這樣的形狀：`reason=review_actions_billing_or_quota`（Copilot review body 只剩 billing 訊息）與 `reason=review_request_failed`（requested_reviewers POST 失敗——app 未安裝、reviewer slug 無效、422）。**允許側的判準同樣是類別而不是這兩個名字**：任何「bot reviewer capability 或 request 本身不可用」的新 reason 同屬允許側。**其餘任何 `UNAVAILABLE` 一律不得 fallback**——判準是類別而不是名單：probe 失敗（`repo_probe_failed`、`pr_probe_failed`、`review_probe_failed`、`requested_reviewer_probe_failed`）、欄位或值的形狀檢查失敗（`*_fields_unparsable`、`*_not_numeric`）、head 無法確認、thread probe／pagination 不完整，以及 helper 日後新增的任何 reason，一律不得 fallback。名字只是例子：前一版是逐條列名，而 `*_fields_unparsable` 整族從未被列入——形狀檢查擋下來的東西會從這個缺口走出去。兩側都是「類別 + 例子」而非名單：允許側只列 reason 名會把 `review_request_failed` 這種「bot capability 根本不可用」的典型形狀關在門外，而 [T0-9] 正是把一般性 bot-UNAVAILABLE 的出口委派給本檔——收窄成單一 reason 會讓本檔交不出那個出口，也讓本節標題（Bot unavailable）與第 1 節「slug／API 不可用時 MUST 標 UNAVAILABLE」失去下游。
