...
3. **Bot unavailable 的 independent fallback**
   ...
   - **允許側**：fallback 只適用於 bot 這一側交不出 review、經合理等待與 retry 後仍為 `UNAVAILABLE`。helper 端屬本類別的是 `reason=review_actions_billing_or_quota`（Copilot review body 逐字是 billing 訊息）與 `reason=review_request_rejected`（`requested_reviewers` POST 回 422 且 GitHub 的 message 指向 collaborator——即 Copilot app 不是本 repo 的 collaborator）。判準是類別（bot capability／request 本身不可用）而不是這兩個名字。**成因分類由 helper 做**：它讀得到 GitHub 回的 message，本檔讀不到（#87）。
   - **排除側**：**除上述允許側的形狀外，helper 回的任何 `UNAVAILABLE` 一律不得 fallback**——`review_request_malformed`（POST 回 422 但 message 不指向 collaborator，即我們這一側的參數錯，MUST 修 helper）、`review_request_probe_failed`（403／404／429／5xx／網路；404 被 GitHub 用來遮蔽權限失敗，與 429／5xx 一樣不可分，MUST retry）、probe 失敗（`repo_probe_failed`、`pr_probe_failed`、`review_probe_failed`、`requested_reviewer_probe_failed`）、欄位或值的形狀檢查失敗（`*_fields_unparsable`、`*_not_numeric`）、head 無法確認、thread probe／pagination 不完整，以及 helper 日後新增而不屬允許側類別的任何 reason，一律不得 fallback。名字只是例子：逐條列名的版本讓 `*_fields_unparsable` 整族從未被列入——形狀檢查擋下來的東西會從那個缺口走出去。（2026-08-24, PR #85／#87）
