...
Every bold term below names a [`GLOSSARY.md`](GLOSSARY.md) heading. Load only the ones the current **branch** needs; a full pruning audit rules every failure mode in or out.
...
Several user-invoked skills that are hard to remember justify one user-invoked **router skill**.
...
1. Ordered actions stay in `SKILL.md` as **steps**, each with a checkable **completion criterion**. Criteria that sweep a set are exhaustive.
2. Rules every branch needs stay as in-skill **reference**.
3. Branch-specific definitions or examples move to a named sibling file behind a **context pointer** that says when to load it.
