...
- `auto-format.sh`、`auto-api-docs.sh`：沒有目標檔案或檔案不存在時立即放行。
...
| auto-format.sh / auto-api-docs.sh | `PostToolUse` | `postToolUse` |
| run-tests.sh（僅有 repo-defined `AGENT_TEST_COMMAND` 時註冊） | `PostToolUse` | `postToolUse` |
...
`hooks.json` 不預設註冊 `run-tests.sh`。只有 repo 明載 focused-test command 時，才加入
`postToolUse` entry，並在 command 設定 `AGENT_TEST_COMMAND`。hook 匯出
`AGENT_TEST_FILE` 與 `AGENT_TEST_ROOT`；不猜 runner、target 或 build state。
