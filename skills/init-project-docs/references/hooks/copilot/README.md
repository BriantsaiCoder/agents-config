...
- `protect-files.sh`：沒有 `tool_input.file_path` / `path` 時立即放行。
...
`protect-files.sh` 偵測到 Copilot host 時，對 stdout 寫 `{"decision":"deny","reason":...}` 並 `exit 0`（見腳本 `hook_block()`）。payload 內 `hook_event_name` 仍可能是 PascalCase，腳本的事件 `case` 需同時相容兩種寫法。
