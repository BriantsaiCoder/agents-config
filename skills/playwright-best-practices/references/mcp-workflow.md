...
4. Before each page close, re-list `browser_tabs` and confirm the task-owned page's current index; then close that index with `action: "close"`. Closing a page shifts later indices, so preserve pre-existing pages and re-observe before the next close. `browser_close` can dispose the MCP backend and close its context/browser, so use it only when all affected resources are task-owned or their closure is explicitly authorized.
