...
7. Before any candidate packet crosses the Phase 2 hunter-to-coordinator handoff or otherwise leaves its source context, the hunter MUST serialize every field, replace credential values and other sensitive literals with `[REDACTED]`, preserve non-sensitive structure, source locations, and relevant set/unset evidence without the value, and add a redaction note to the evidence for every affected field path without copying the raw literal.
```
