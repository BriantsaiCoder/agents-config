...
description: Use when writing or reviewing Dockerfiles for any stack — multi-stage builds, .dockerignore, Linux/Windows containers, ASP.NET Core, .NET Framework + IIS, Node.js, Python, Go, Java. Apply even when user just says "containerize this app", "make the image smaller", "fix Docker build cache", or "switch to non-root" without naming Docker.
---
