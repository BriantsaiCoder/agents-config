...
5. **Resolve research tickets.** Apply shared `dev-workflow` INT-4: delegate only substantial, independent research tickets when doing so materially helps; use one subagent when it can cover related tickets, otherwise resolve them in the main context. Capture each result at the ticket's authorized output location; do not create a throwaway branch unless the delivery contract calls for one.
