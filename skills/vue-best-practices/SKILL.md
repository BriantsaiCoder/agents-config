...
3. **`defineProps<T>()`** — compile-time check, no runtime overhead.
...
6. **Cleanup `watch` / `watchEffect`** with `onCleanup` — leaks otherwise.
...
- `references/component-patterns.md` — SFC, Props/Emits, Slots, async, Teleport, `defineModel`
- `references/state-management.md` — `ref` vs `reactive`, `computed`, `watch`, VueUse, provide/inject
