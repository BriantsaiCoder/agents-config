...
When this fallback contract is selected, every triage comment or issue starts with:

```text
> *This was generated by AI during triage.*
```

A repository tracker contract or explicit user/team instruction owns any different disclosure requirement. Disclosure does not authorize publishing; prepare a draft until the specific tracker action is authorized.
