...
| A6 | 路由丟第二跳 | ✅ | 補完 `diagnosing-bugs` → `writing-great-skills` 兩跳，並宣告 `dev-workflow` 路由表為權威 |
| A7 | 無跨 skill collision step | ✅ | 新增 Step 2b（唯一比較 skill 之間的一步）+ Red Flags 一列 + Iron Law 納入 |

**順帶修掉的 doc rot**：`step0-vendored-gate.md` 與 `ci.yml` 都宣稱 `vendored-detection.sh` 是「33 cases」，實際 **91**。

**副作用（如實記錄）**：`SKILL.md` 661 → 1016 words（limit 500）。它**改動前就已 OVER**；淨增 355 換得 6 項實質補強。中途曾到 1137，依 `step1-verdict-guide.md` 定位成因（我加的是規則＋理由，而理由在 satellite 已有完整版）後壓回。再往下砍就是砍載重規則，即該指南警告的「Trim 丟失資訊」，故停在此並列為後續項。
...
- tree SHA-256 `6d00cd49…` → **`0050ffd7d068e864ba0d32be6cf82254599507a69c6eb69d76ceda62795a028e`**，已驗證實際＝紀錄
...
新增測試 2 支、已接 CI：`tests/description-lint.sh`（22 cases）、`tests/word-budget.sh`（26 cases）。
...
- **未 commit。** `~/.agents` 在 `main`，依家規需使用者確認。
