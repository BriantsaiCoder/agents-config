...
> 修訂：2026-07-29；依使用者「核准 candidate-only maintenance .in_use active-owner amendment 與 RED→GREEN 驗證」修正 Claude maintenance quiescence gate。
...
8. 只從 skill-directory enumeration排除 exact top-level `skills/.claude` runtime directory；該directory仍留在shared-skills manifest與metadata gate。禁止 hidden-directory wildcard；其他hidden directory仍須缺`SKILL.md`即fail-fast。
...
Post-rollback T2-6 preservation amendment：

- current T2-6只保留到三家host-local active config；不得恢復 `.agents` control-plane ownership。
- shared candidate `core/tier2-style.md`只承接current exact bytes作為transition／rollback carrier；三家active config不得引用它。
- Claude materialized `core/tier2-style.md`承接相同exact bytes，但active enforcement仍由thin `CLAUDE.md`明列。
- Claude、Codex、Copilot的active root config各自包含同一條response-policy語意；不得以某host結果推論其他host。
- live generated dist／manifest不是新architecture的active config；candidate-only階段不得藉此更新live baseline或carrier。
...
- [ ] **Step H3: Build plugin-disabled shadow probe**
  ...
  Copilot carrier的`config.json`含absolute `installedPlugins[*].cache_path`；設定scratch `COPILOT_HOME`不會自動改寫這些embedded paths。Copilot shadow probe必須先用`bin/copilot-plugin-simulation-config`將每個exact live-home prefix改到simulation home，並確認所有rewritten cache directories都存在；任何non-live-prefix path立即`UNAVAILABLE`，不得部分改寫後繼續。
...
### 13.1 Claude `.in_use` active-owner gate

Claude plugin cache下的`.in_use`是跨plugin／version持久存在的marker container；directory存在不等於runtime仍持有plugin。不得以 `.in_use` directory count = 0作為maintenance gate，directory count只可作metadata row監看。

Claude maintenance quiescence必須同時滿足：Claude process count = 0、active marker-owner intersection = 0、兩個exact Superpowers trees open-handle count = 0。

Active marker-owner判定：

1. 只讀列舉exact `~/.claude/plugins/cache/**/.in_use/*` marker；從檔名解析PID，非空JSON另驗`pid`與檔名一致，`procStart`存在時另與process start比對；
2. PID不存在為stale；PID存在且`procStart`一致為active；PID存在但缺少可比對的`procStart`、marker格式不合法或process probe不可用皆為`UNAVAILABLE`，不得假綠；
3. open-handle probe只允許兩個exact paths：`~/.claude/plugins/cache/claude-plugins-official/superpowers/6.2.0`與`~/.claude/plugins/cache/superpowers-marketplace/superpowers/6.2.0`；禁止plugin-name、version或hidden-path wildcard；
4. pre-write出現active owner／`FAIL`／`UNAVAILABLE`立即停止；transaction已有write時則立即執行coordinated rollback。

禁止刪除、清空或wildcard處理任何 `.in_use`；stale marker保持原byte／path／mode，不納入semantic plugin payload mutation。

### 13.2 Copilot simulation `cache_path` isolation gate

Copilot restore carrier的`config.json`可保存live absolute `installedPlugins[*].cache_path`。不得只因 `COPILOT_HOME` 指向scratch就宣稱隔離；native uninstall會依embedded path操作cache。

任何Copilot retirement simulation必須：

1. 將Copilot control-plane與Superpowers carriers解到`mktemp -d`內的simulation home，再依source config逐一把所有registered cache payload以read-only source copy materialize到rewritten relative path；禁止用symlink或absolute reference代替；
2. 執行`bin/copilot-plugin-simulation-config <simulation-home>/config.json <simulation-home> <live-home>`；helper只接受exact simulation config，只移除JSONC full-line `//` comments後交由`jq`解析（其他非JSON syntax fail-closed），要求每個source `cache_path`都以exact live-home prefix開頭、只做prefix replacement、要求rewritten physical directory已存在且沒有symlink escape，並以mode 0600 atomic replace；
3. 以`jq -e`另驗所有simulation config的`cache_path`都在simulation home且沒有live-home prefix；不得輸出config body；
4. simulation前後live Copilot plugin inventory與exact cache tree manifest必須empty diff；simulation inventory必須由present變absent；
5. simulation只可設定scratch `COPILOT_HOME`與`COPILOT_AUTO_UPDATE=false`，禁止SaaS／remote／credential讀取。

任一條件`FAIL`／`UNAVAILABLE`即停止，不得執行live uninstall；若transaction已有write，立即coordinated rollback。
...
> **使用者另行明示授權建立fresh post-failed-transaction live baseline與新full rollback carrier並read-only驗證；完成後仍需重新明示授權coordinated live cutover retry與三家Superpowers實際退休，maintenance window後的fixed 6-run canary仍需獨立明示授權。**
