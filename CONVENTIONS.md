# ~/.agents AI 讀者書寫規範（13 條）

> 讀者是 AI 模型，不是人。原則只有一個：**每一句話都要能被機械驗證或機械執行**——模型換代後散文語感會漂移，但「檔案存在」「grep 命中」「exit code」不會。Shared skills 由本 repo tests 驗證；host global config 由各 host repo 自己驗證。

## 1. 檔頭 metadata（HTML comment，不佔注入 token 預算）

`skills/**` 的 metadata 依各 skill contract；host global config 的 metadata 由 `~/.claude`、`~/.codex`、`~/.copilot` 各自管理。`core/` 與已退役的 `attic/hosts/`、`attic/dist/` 的舊 generated metadata 只作 historical／rollback evidence，不代表 active ownership。

## 2. 規則五要素格式

每條規則單行自足：`[T0-3] MUST NOT force-push 到 main/master。觸發：git push --force* 且目標分支為 main|master。例外：無。驗證：rules lint + hook audit-bash。` 五要素 = 穩定 ID + RFC2119 動詞（MUST / MUST NOT / SHOULD）+ 機械觸發條件 + 明示例外（沒有就寫「無」）+ 驗證方式。缺任一要素不得進 tier 檔。

## 3. 規則 ID 永不重編、永不回收

刪除或降級規則時留殼標記：`[T1-7 DEPRECATED→T0-2 2026-07]`。理由：舊 session transcript、commit message、cookbook 條目可能引用舊 ID；重編會讓歷史引用全部斷鏈且無人察覺。

## 4. 跨檔引用只用規則 ID 或檔路徑

禁止散文錨點（「見上方 merge gate 一節」「如前述」）。散文錨點在檔案被生成器重組後靜默失效；規則 ID 與路徑可被 lint 驗證。（實證：DCT repo doc-rot 審計中錨點解析器掃不到散文符號殘留，須另跑 grep 雙軌。）

## 5. 載入宣稱必附驗證指令

任何「X 會載入 Y」「X 繼承 Y」的句子，同段必附可執行探針指令 + 預期輸出。無法驗證的宣稱必須加 `UNVERIFIED:` 前綴且禁用肯定句式。（實證：Copilot「已自動載入 ~/.claude/CLAUDE.md」一句未驗證肯定句，讓完整 Hard Rules 失守一個月，見 proposals/01 F1。）

## 6. Fingerprint codeword

每個常駐注入檔含恰一句唯一指紋，格式 `FP:<檔名代號>-<年季>`，例 `FP:AGENTS-T0-2026Q3`。用途：context 級載入驗證。檔案級驗證（symlink 存在、hash 相符）不足以證明「進了 context」——兩起事故皆是檔案層正常、context 層死透。

## 7. 名稱引用必過 lint

skill / agent / MCP server / hook 名稱一律 lint 對照實體目錄；手寫清單禁止，索引只准機械生成。反引號引用的 skill / command 名必須存在於 `skills/` 或明標 `host:<name>-only` 豁免。（實證：Copilot 9 個死 skill 引用、`chrome-devtoolss` 拼錯，全是手寫清單腐爛。）

## 8. Context 硬預算

各 host 的 tier0 ≤30 行、routing ≤30 行、常駐 global config ≤10KB；超標由 host-local test fail。理由：Codex override 事故根因之一是「32KB 太長→有人另立短檔→短檔整檔取代長檔」。預算是防止下一次 override 誕生的結構性手段。

## 9. why 一句話 + 出處座標；可機械化者必須下沉

規則的 why 限一句 + 座標（commit SHA / PR 編號 / 審計日期），詳細敘事放 cookbook 或 proposal 附件。凡能寫成 hook / lint / test 的檢查，必須下沉為機械守護並從 prose 移除——prose 只留判斷類知識。

## 10. 語言與句式

zh-TW；術語照附錄 A 用詞對照表（建立／物件／佇列；禁「創建／質量／視圖」）；規則句禁隱喻、禁反問；每句單獨抽出仍可執行（模型可能只看到被 grep 出來的單句）。技術名詞與識別字保留原文。

## 11. 版控取代 .bak（`~/.agents` git 化後生效）

三家設定目錄（`~/.claude`、`~/.codex`、`~/.copilot`）的手工 `.bak` 慣例廢止：設定變更一律走 git commit；MUST NOT 再產生 `*.bak*`。既有 .bak 掃 secret 後刪除或歸檔 `attic/`。（實證 2026-07-07：`~/.codex` 累積 .bak，其中 3 份 `config.toml.bak` 額外複製了同一明文 API key——備份檔是 secrets 殘留的最大死角。）驗證（兩式都要 0）：`find ~/.claude ~/.codex ~/.copilot -maxdepth 1 -name '*.bak*' -not -name '.*' 2>/dev/null | wc -l` 與 `find ~/.agents -name '*.bak*' -not -path '*/.git/*' -not -name '.*' 2>/dev/null | wc -l`。例外：app 自動生成的 runtime state 備份（如 `.codex-global-state.json.bak`，dotfile 開頭、app 自管生命週期）不算違規，不得手動刪除——`-not -name '.*'` 就是該例外的機械化。（2026-07-30 修：原式用 `ls` + glob，zsh 下任一目錄無匹配即 nomatch 中止並回 0＝假合規，實測同一條指令 zsh 回 0、bash 回 4；且只掃 host 根目錄 maxdepth 1，`~/.agents/core` 與 `~/.agents/rules` 的兩個 .bak 因此隱形 12 天。驗證式自身會假陰性時，規則等於不存在。）

**`attic/` 與 `backups/` 各有其一，不可互換**（2026-07-26 定義；此前兩者並存無成文分工，每次歸檔都要重猜一次）：

| 目錄 | 放什麼 | 版控 | 命名 | 生命週期 |
|---|---|---|---|---|
| `attic/` | **退役物**——決定不再使用、但保留供考古的完整資產（退役 skill、被取代的 host 設定檔） | **進版控**，用 `git mv` 保留歷史 | 沿用原名（`attic/ecpay`、`attic/design-doc-mermaid`） | 長期；刪除需與當初退役決定同級的理由 |
| `backups/` | **操作前快照**——某次批次變更動手前的狀態，用來回退或事後比對 | **不進版控**（`.gitignore:9`），純本機 | `YYYYMMDD-<操作代號>/`（`20260725-p0a-guard`） | 短期；該批變更驗證通過且已進 git 後即可清 |

判準一句話：**「這東西以後還會不會被當成正本讀？」不會 → `attic/`；「這是某次操作的還原點嗎？」是 → `backups/`。** 退役一個 skill 走 `attic/` 並在 `vendored-forks.md` 或 commit message 留還原路徑；批次改設定前的快照走 `backups/`。

版控狀態的差異不是疏漏而是分工的一部分：`attic/` 要能被未來的人 `git log --follow` 追到來歷，所以進版控；`backups/` 的還原機制本來就是 git 本身，快照只是操作當下的方便，進版控只會讓 repo 膨脹。**推論：`backups/` 裡的東西一旦本機刪掉就沒了——凡是刪掉會後悔的，它就不屬於 `backups/`，屬於 `attic/`。** 驗證（在 `~/.agents` 跑，worktree 內看不到 gitignored 目錄）：`ls ~/.agents/backups/` 每一項都符合 `YYYYMMDD-` 前綴；`git -C ~/.agents ls-files backups/ | wc -l` = 0。

## 12. Claude 常駐面預算（CLAUDE.md + core 合計）

`~/.claude/CLAUDE.md` 與三個 Claude-local core 檔合計 ≤20KB（量測：`cat ~/.claude/CLAUDE.md ~/.claude/core/tier{0,1,2}-*.md | wc -c`）。觸發：編輯 CLAUDE.md 或 Claude core。理由：2026-07-08 審計（F7）發現 CLAUDE.md 與 tier0-2 逐句重複 ~6KB——重複不只費 token，更製造 drift 面。CLAUDE.md 只放 Claude 專屬語意；與 tier 規則重疊者一律刪除改 rule-ID 引用。例外：無。驗證：量測式 ≤20480；`grep -c "原生（語言 / 框架" ~/.claude/CLAUDE.md` = 0（抽樣重複片語）。

## 13. 注入層加一刪一（≥90% 預算時生效）

任一 host 常駐面 ≥ 預算 90%（10KB 預算即 9216B）時，新增常駐行 MUST 在同批刪除等量重複／過時行補償，不得單向膨脹。觸發：host-local size test 顯示 ≥90%。理由：預算是防 override 的結構性手段（規則 8），逼近上限時「先加後刪」會讓 build fail 擋住合法修正。例外：安全紅線（tier0）新增可先入，同批補刪其他層。驗證：host-local test 通過且常駐面未淨增。


---

# 附錄（非注入常駐預算；供 AI on-demand 查閱，不進三家 host context）

> 由 tier2 遷入（原佔每家常駐預算 ~836B）。這兩份是「查閱型參考」非「每回合必駐規則」：用詞紅線本體 [T2-5] 已在 tier2 並指向附錄 A；Code Style（附錄 B）屬判斷類、無 tier 規則錨（不機械強制），完整對照與原則存此供 on-demand 查閱。

## 附錄 A：用詞對照（一律台灣慣用詞；T2-5 完整表）

create=建立、object=物件、queue=佇列、stack=堆疊、information=資訊、code=程式碼、running=執行、library=函式庫、building=建構、package=套件、class=類別、function=函式、memory=記憶體、built-in=內建、global=全域、example=範例、quality=品質、document=文件、file=檔案、data=資料、component=元件、integration=整合、search=搜尋、store=儲存

## 附錄 B：Code Style 判斷原則（app code；判斷類不機械強制，非本檔 13 條規範）

Favor pure functions；query 與 command 分離（CQS）｜Fail fast，不靜默吞錯｜DRY 第 3 次重複才抽（Rule of Three）｜YAGNI，不先建不必要抽象｜Composition > inheritance｜命名表 intent，註解解釋 why｜低耦合高內聚，外部依賴用 DI｜巢狀深度 ≤ 2–3 層，超過就抽方法。
